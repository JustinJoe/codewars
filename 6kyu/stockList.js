/* 
A bookseller has lots of books classified in 26 categories labeled A, B, ... Z. Each book has 
a code c of 3, 4, 5 or more characters. The 1st character of a code is a capital letter which 
...
// P:
function stockList(listofStock, listOfCategories) {
  // split categories from stock and their quantities into an object
  // if category exists already add up

  let categories = {};

  if (!listofStock.length) return "";

  listofStock.map((category) => {
    let cat = category[0];
    categories[cat] = (categories[cat] | 0) + +category.split(" ")[1];
  });
